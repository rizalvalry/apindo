...
function getFileOrginal($image, $clean = '')
{
    return file_exists($image) && is_file($image) ? asset($image) . $clean : asset(config('location.default'));
}
...
function getFile($disk = 'local', $image = '')
{
    try {
        if ($disk == 'local') {
            $localImage = asset('/assets/uploads') . '/' . $image;
...
function getCookieInfo(){
    $sectionName = 'cookie-consent';
    $cookie = Template::where('section_name', $sectionName)->first();
    if($cookie){
        return $cookie;
    }else{
        return false;
