...
class BasicService
{
    ...
    public function preparePaymentUpgradation($order)
    {
        ...
        if ($order->status == 0) {
            ...
            $packageName = optional($order->purchasePackage->get_package)->title??'unknown';
            if($order->purchasePackage->type == 'Purchase'){
                $type ="Buy " .$packageName;
            }else{
                $type ="Renew " .$packageName;
            }

            $this->makeTransaction($user, getAmount($order->amount), getAmount($order->charge), '-',  $order->transaction, $gateway->name, $order->id,$type);

            $purchasePackage = $order->purchasePackage;
     ...
     * @param $remarks
     */
    public function makeTransaction($user, $amount, $charge, $trx_type = null, $trx_id, $remarks = null, $fund_id = null,$type=null): void
    {
        $transaction = new Transaction();
        $transaction->user_id = $user->id;
        $transaction->fund_id = $fund_id??null;
