...
trait ListingTrait
{
        ...
        for ($i = 0; $i < $numberOfImgPerListing; $i++) {
            if ($request->hasFile("listing_image.$i") && $request->file("listing_image.$i")->getSize() <= 5242880) {
                try {
                    ...
                    $image = $this->fileUpload($request->listing_image[$i], config('location.listing.path'), $listingImage->driver, null);
                    if ($image) {
        ...
        for ($i = 0; $i < $numberOfProductsPerListing; $i++) {
            try {
                ...
                if ($request->hasFile("product_thumbnail.$i") && $request->file("product_thumbnail.$i")->getSize() <= 5242880) {
                    $image = $this->fileUpload($request->product_thumbnail[$i], config('location.product_thumbnail.path'), $product->driver, null, $product->product_thumbnail);
                    if ($image) {
                ...
                for ($j = 0; $j < $imageCount; $j++) {
                    if ($request->hasFile("product_image.$newKey.$j")) {
                        try {
                            ...
                            $image = $this->fileUpload($request->product_image[$newKey][$j], config('location.product.path'), $product_image->driver, null, $product_image->product_image);
                            if ($image) {
    ...
    public function insertAmenitites($numberOfAmenitiesPerListing, Request $request, $listing, $id)
    {
        ...
        DB::table('listing_aminities')->insert($amenities);
    }
    ...
    public function insertSEO($listing, Request $request, $id)
    {
        try {
            ...
            if ($request->hasFile('seo_image')) {
                ...
                $image = $this->fileUpload($request->seo_image, config('location.listing_seo.path'), $listingSeo->driver, null);
                if ($image) {
