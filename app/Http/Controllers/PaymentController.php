...
class PaymentController extends Controller
{
    ...
    public function depositConfirm(Request $request)
    {
        ...
        if (is_null($order)) {
            return redirect()->route('user.addFund')->with('error', __('Invalid Fund Request'));
        }
        if ($order->status != 0) {
            return redirect()->route('user.addFund')->with('error', __('Invalid Fund Request'));
        }
    ...
    public function fromSubmit(Request  $request)
    {
        ...
        session()->flash('success', 'You payment request has been taken! You will be notify very soon.');
    ...
    public function success(Request $request)
    {
        ...
        $renewPackage = session()->get('renewPackage');
        if ($renewPackage == 'package_confirm'){
            $packageName = session()->get('packageName');
            return redirect()->route('user.myPackages')->with('success', '`'.$packageName.'`'. ' has been purchased successfully.');

        }else{
            $packageName = session()->get('packageName');
            return redirect()->route('user.myPackages')->with('success', '`'.$packageName.'`'. ' has been renewed successfully.');
        }
     ...
     * @return $amount
     */
   static public function newFund(Request $request, $user, $gate, $charge, $final_amo, $amount,$purchasePackage=null, $remarks=null): Fund
    {
        $fund = new Fund();
        $fund->user_id = $user->id;
        $fund->gateway_id = ($gate) ? $gate->id : null;
        $fund->purchase_package_id = $purchasePackage??null;
        $fund->gateway_currency = ($gate) ? strtoupper($gate->currency) : null;
        $fund->amount = $amount;
        $fund->charge = $charge;
        $fund->rate = ($gate) ? $gate->convention_rate : null;
