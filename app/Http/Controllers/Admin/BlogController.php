...
class BlogController extends Controller
{
    ...
    public function blogStore(Request $request, $language = null)
    {
        ...
        if ($request->hasFile('image')) {
            try {
                $image = $this->fileUpload($request->image, config('location.blog.path'), $blog->driver, null, $blog->image);
                if ($image) {
                    $blog->image = $image['path'];
                    $blog->driver = $image['driver'];
                }
            } catch (\Exception $exp) {
    ...
    public function blogUpdate(Request $request, $id, $language_id)
    {
        ...
        if ($request->hasFile('image')) {
            try {
                $image = $this->fileUpload($request->image, config('location.blog.path'), $blog->driver, null);
                if ($image) {
                    $blog->image = $image['path'];
                    $blog->driver = $image['driver'];
                }
    ...
    public function blogDelete($id)
    {
        ...
        if (!empty($old_image)) {
            $this->fileDelete($blog->driver, $old_image);
        }
