...
class UsersController extends Controller
{
    ...
    public function activeMultiple(Request $request)
    {
        if ($request->strIds == null) {
            ...
            session()->flash('success', __('User Status Has Been Active'));
    ...
    public function inactiveMultiple(Request $request)
    {
        ...
        if ($request->strIds == null) {
            session()->flash('error', __('You do not select User.'));
            return response()->json(['error' => 1]);
        } else {
            User::whereIn('id', $request->strIds)->update([
                'status' => 0,
            ...
            session()->flash('success', __('User Status Has Been Deactive'));
            return response()->json(['success' => 1]);
        }
    }
    ...
    public function userUpdate(Request $request, $id)
    {
        ...
        if ($request->hasFile('image')) {
            try {
                $old = $user->image ?: null;
                $image = $this->fileUpload($request->image, config('location.user.path'), $user->driver, null, $old);
                if ($image) {
    ...
    public function userBalanceUpdate(Request $request, $id)
    {
        ...
        if ($userData['balance'] == null) {
            return back()->with('error', __('Balance Value Empty!'));
        } else {
            ...
            if ($userData['add_status'] == "1") {
                ...
                return back()->with('success', __('Balance Add Successfully.'));

            } else {

                if ($userData['balance'] > $user->balance) {
                    return back()->with('error', __('Insufficient Balance to deducted.'));
                }
                $user->balance -= $userData['balance'];
                $user->save();
                ...
                return back()->with('success', __('Balance deducted Successfully.'));
            }
        }
    }
    ...
    public function kycAction(Request $request, $id)
    {
        ...
        if ($request->status == '1') {
            ...
            session()->flash('success', __('Approve Successfully'));
            return back();
        ...
        elseif ($request->status == '2') {
            ...
            session()->flash('success', __('Reject Successfully'));
            return back();
        }
    }
}
