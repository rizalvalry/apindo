...
class ManualGatewayController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            try {
                $image = $this->fileUpload($request->image, config('location.gateway.path'), $getGateway->driver, null);
                if ($image) {
        ...
        try {
            ...
            return back()->with('success', __('Gateway data has been saved.'));
    ...
    public function update(Request $request, $id)
    {
        ...
        if(1000 > $getGateway->id){
            return back()->with('error', __('Invalid Gateways Request'));
        }
        ...
        try {
            ...
            $res = $getGateway->save();
            if (!$res) {
                throw new \Exception(__('Unexpected error! Please try again.'));
            }
            return back()->with('success', __('Gateway data has been updated.'));
