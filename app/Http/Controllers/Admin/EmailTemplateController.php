...
class EmailTemplateController extends Controller
{
    ...
    public function emailConfigure(Request $request)
    {
        ...
        $env = file($envPath);
        $env = $this->set('MAIL_FROM_ADDRESS', '"' . $request->sender_email . '"', $env);
        $env = $this->set('MAIL_FROM_NAME', '"' . $request->sender_email_name . '"', $env);

        $env = $this->set('MAIL_MAILER', config('basic.email_configuration.name'), $env);
        if (config('basic.email_configuration.name') == 'smtp') {
            $env = $this->set('MAIL_HOST', '"' . $request->smtp_host . '"', $env);
            $env = $this->set('MAIL_PORT', '"' . $request->smtp_port . '"', $env);
            $env = $this->set('MAIL_ENCRYPTION', '"' . $request->smtp_encryption . '"', $env);
            $env = $this->set('MAIL_USERNAME', '"' . $request->smtp_username . '"', $env);
            $env = $this->set('MAIL_PASSWORD', '"' . $request->smtp_password . '"', $env);
        }
        $fp = fopen($envPath, 'w');
        ...
        foreach ($emailTemplate as $template) {
            $template->email_from = $request->sender_email;
            $template->save();
        }
