...
class TicketController extends Controller
{
    ...
    public function ticketDownload($ticket_id)
    {
        ...
        $disk = $attachment->driver;
        if ($disk == 'local') {
            $localImage = ('assets/uploads') . '/'.$path.'/' . explode('/',$file)[1];
            $full_path = Storage::disk($disk)->exists($file) ? $localImage : null;
        } else {
            $full_path = Storage::disk($disk)->exists($file) ? Storage::disk($disk)->url($file) : null;
        }
    ...
    public function ticketDelete(Request $request)
    {
        ...
        if (count($message->attachments) > 0) {
            foreach ($message->attachments as $img) {
                $this->fileDelete($img->driver, $img->image);
                $img->delete();
            }
        }
        $message->delete();
        return back()->with('success', __("Message has been deleted"));
    }
}
