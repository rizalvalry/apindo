...
class TemplateController extends Controller
{
    ...
    public function update(Request $request, $section, $language)
    {
        ...
        if ($request->hasAny(array_keys(config('templates.template_media')))) {
            ...
            foreach (config('templates.template_media') as $key => $media) {
                ...
                if ($request->hasFile($key)) {
                    $image = $this->fileUpload($purifiedData[$key][$language], config('location.template.path'), $templateMedia->driver, null, $old_data);
                    if ($image) {
        ...
        return back()->with('success', __('Template Successfully Saved'));
    }
