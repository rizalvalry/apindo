...
class VerificationController extends Controller
{
        ...
        elseif (!$user->email_verification) {
            if (!$this->checkValidCode($user, $user->verify_code)) {
                ...
                $this->mailVerification($user, 'VERIFICATION_CODE', [
                  ...
                  session()->flash('success', __('Email verification code has been sent'));
            }
        ...
        elseif (!$user->sms_verification) {
            if (!$this->checkValidCode($user, $user->verify_code)) {
                ...
                $this->smsVerification($user, 'VERIFICATION_CODE', [
                 ...
                 session()->flash('success', __('SMS verification code has been sent'));
            }
    ...
    public function resendCode()
    {
        ...
        if ($type === 'email') {
            ...
            return back()->with('success', __('Email verification code has been sent'));
        } elseif ($type === 'mobile') {
            $this->smsVerification($user,'VERIFICATION_CODE',[
                'code' => $user->verify_code
            ]);
            return back()->with('success', __('SMS verification code has been sent'));
        } else {
    ...
    public function mailVerify(Request $request)
    {
        ...
        $msg = [
            'code.required' => __('Email verification code is required'),
        ];
