                        ...
                        <div class="row gy-4">
                            <div class="col-lg-6">
                                <div class="about d-md-flex">
                                    <img src="{{ getFile(optional($user_information)->driver, optional($user_information)->image) }}" class="img-fluid profile" alt="{{config('basic.site_title')}}"/>
                                    <div>
                                        <h4 class="name">
                                        ...
                                        <div class="links">
                                            ...
                                            @if($user_information->address)
                                                <a href="javascript:void(0)"><i class="fas fa-location-arrow"></i>@lang($user_information->address)</a>
                                            @endif
                                            @if($user_information->created_at)
                                                <a href="javascript:void(0)" ><i class="fas fa-calendar-alt"></i>@lang('Joined'){{ $user_information->created_at->format('M Y') }}</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="button-group">
                                        ...
                                        @if(Auth::check() == true)
                                            @if(count($check_follower) < 1)
                                                <form action="{{ route('user.follow', $user_information->id) }}" method="post" class="d-inline-block">
                                                    @csrf
                                            ...
                                            @else
                                                <form action="{{ route('user.unFollow', $user_information->id) }}" method="post" class="d-inline-block">
                                                    @csrf
                                                    <button class="btn-custom follow-btn disabled cursor-follow-btn" type="submit" >
                                            ...
                                            @endif
                                        @else
                                            <form action="{{ route('user.unFollow', $user_information->id) }}"
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="followers">
                        ...
                        @forelse($user_information->follower as $follower)
                            <div class="follower">
                                <a href="{{ route('profile', [slug(optional($follower->get_follwer_user)->firstname), optional($follower->get_follwer_user)->id]) }}">
                                    <img src="{{ getFile(optional($follower->get_follwer_user)->driver, optional($follower->get_follwer_user)->image) }}" class="img-fluid" alt="@lang('follower')"/>
                                </a>
                                <div class="creator-box">
                                    <div class="img-box">
                                        <img src="{{ getFile(optional($follower->get_follwer_user)->cover_driver, optional($follower->get_follwer_user)->cover_photo) }}" alt="{{config('basic.site_title')}}" class="img-fluid cover"/>
                                        <img src="{{ getFile(optional($follower->get_follwer_user)->driver, optional($follower->get_follwer_user)->image) }}" class="img-fluid profile" alt="{{config('basic.site_title')}}"/>
                                    </div>

                                    <div class="text-box">
                                        <a class="creator-name" href="{{ route('profile', [slug(optional($follower->get_follwer_user)->firstname), optional($follower->get_follwer_user)->id]) }}">
                                            @lang(optional($follower->get_follwer_user)->firstname) @lang(optional($follower->get_follwer_user)->lastname)
                                        </a>
                                        <span>@lang('Member since') {{ optional($follower->get_follwer_user)->created_at->format('M Y') }}</span>
                                    </div>
                                </div>
                            </div>
                        @empty
                    ...
                    <div class="followers">
                        ...
                        @forelse($user_information->following as $following)
                            <div class="follower">
                                <a href="{{ route('profile', [slug(optional($following->get_following_user)->firstname), optional($following->get_following_user)->id]) }}">
                                    <img src="{{ getFile(optional($following->get_following_user)->driver, optional($following->get_following_user)->image) }}" class="img-fluid" alt="@lang('follower')"/>
                                </a>
                                <div class="creator-box">
                                    <div class="img-box">
                                        <img src="{{ getFile(optional($following->get_following_user)->cover_driver, optional($following->get_following_user)->cover_photo) }}" alt="{{config('basic.site_title')}}" class="img-fluid cover"/>
                                        <img src="{{ getFile(optional($following->get_following_user)->driver, optional($following->get_following_user)->image) }}" class="img-fluid profile" alt="{{config('basic.site_title')}}"/>
                                    </div>
                                    <div class="text-box">
                                        <a class="creator-name" href="{{ route('profile', [slug(optional($following->get_following_user)->firstname), optional($following->get_following_user)->id]) }}">
                                            @lang(optional($following->get_following_user)->firstname) @lang(optional($following->get_following_user)->lastname)
                                        </a>
                                        <span>@lang('Member since') {{ optional($following->get_following_user)->created_at->format('M Y') }}</span>
                                    </div>
                                </div>
                            </div>
                        @empty
                ...
                <div class="col-lg-4">
                    <div class="side-box">
                        ...
                        <form action="{{ route('user.viewerSendMessageToListingUser', $user_information->id) }}" method="post"
...
//                                        $allCategories = $latest_listing->getCategories($categoryIds);
                                    ...
                                    <div class="col-lg-4 col-md-6">
                                        <div class="listing-box">
                                            <div class="img-box">
                                                <img class="img-fluid" src="{{ getFile($latest_listing->driver, $latest_listing->thumbnail) }}" alt="{{config('basic.site_title')}}"/>
                                                <button class="save wishList" type="button" id="{{$key}}" data-user="{{ optional($latest_listing->get_user)->id }}" data-purchase="{{ $latest_listing->purchase_package_id }}" data-listing="{{ $latest_listing->id }}">
                                            ...
                                            <div class="text-box">
                                                ...
                                                <a class="title" href="{{ route('listing-details', [slug($latest_listing->title), $latest_listing->id] ) }}">
                                                ...
                                                <p class="address mt-1">
                                                    ...
                                                    @lang($latest_listing->address) , @lang(optional(optional($latest_listing->get_place)->details)->place)
                                                </p>
                                            </div>
                                        </div>
                                    </div>
