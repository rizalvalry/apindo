...
@if(isset($templates['maintenance-page'][0]) && $maintenancePage = $templates['maintenance-page'][0])
    <div class="maintenance">
        <div class="maintenance_contain">
            <img src="{{getFile(optional($maintenancePage->media)->driver, optional($maintenancePage->templateMedia())->image)}}"
                 alt="@lang('maintenance')">
            <h5 class="pp-infobox-title-prefix m-0">@lang(optional($maintenancePage->description)->title)</h5>
                ...
                <div class="social-links mt-3">
                    <div class="pp-social-icons pp-social-icons-center pp-responsive-center">
                        @foreach($contentDetails['social'] as $data)
                            ...
                            <link itemprop="url"
                                  href="{{optional(optional(optional($data->content)->contentMedia)->description)->link}}">
                            <a itemprop="sameAs"
                               href="{{optional(optional(optional($data->content)->contentMedia)->description)->link}}"
                               target="_blank" title="{{optional($data->description)->name}}" aria-label="Facebook"
                               role="button">
