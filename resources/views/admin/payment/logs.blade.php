    ...
    <div class="page-header card card-primary m-0 m-md-4 my-4 m-md-0 p-5 shadow">
        <form action="{{ route('admin.payment.search') }}" method="get">
            <div class="row justify-content-between align-items-center">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <select name="status" class="form-control">
                            <option value="" selected disabled>@lang('Select Payment')</option>
                            <option value="1"
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <input type="date" class="form-control" name="date_time" id="datepicker" value="{{ old('date_time',request()->date_time) }}"/>
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="form-group">
                        ...
                        <tr>
                            ...
                            <td data-label="@lang('Username')">
                                <div class="float-left">
                                    <a href="{{route('admin.user-edit',optional($fund->user)->id)}}" target="_blank">
                                        <img src="{{ getFile(optional($fund->user)->driver, optional($fund->user)->image) }}" alt="{{config('basic.site_title')}}" class="contactImageUser">
                                    </a>
                                </div>
                                <div class="float-left">
                                    @lang(optional($fund->user)->username) <br>
                                    @lang(optional($fund->user)->email)
                                </div>
                            </td>

                            <td data-label="@lang('Method')">{{ ($fund->gateway == null ? __('N/A') : __(optional($fund->gateway)->name)) }}</td>
        ...
        $(document).ready(function () {
            ...
            $(document).on("click", '.edit_button', function (e) {
                ...
                details.map(function (item, i) {
                    if (item[1].type == 'file') {
                        var singleInfo = `<br><img src="${item[1].field_name}" alt="{{config('basic.site_title')}}" class="w-100">`;
                    } else {
