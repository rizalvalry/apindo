                        ...
                        <td data-label="@lang('User')">
                            <div class="float-left">
                                <a href="{{route('admin.user-edit',$row->user_id)}}" target="_blank">
                                    <img src="{{getFile(optional($row->user)->driver, optional($row->user)->image)}}" class="contactImageUser">
                                </a>
                            </div>

                            <div class="float-left">
                                @lang($row->user->username) <br>
                                @lang($row->user->email)
                            </div>

                        </td>
                        <td data-label="@lang('Amount')"> <span class="text-{{($row->trx_type == "-") ? 'success' :'primary'}}">{{config('basic.currency_symbol')}}{{getAmount($row->amount, config('basic.fraction_number'))}}</span></td>
                        <td data-label="@lang('Gateway')">@lang($row->remarks)</td>
                        <td data-label="@lang('Remark')">@lang($row->type)</td>
